...
from common.product import image_chat_answer, load_huggingface_embedding, product_vector_index, save_product_ratings, product_index_directory

from utils import init_page_header, init_session_state, get_config, get_avatar
...
def save_product_info(id, name, title, tags, image, video, images, gender, season, price, style, material, advantage, marketing, description):
    try:
        ...
        with engine.connect() as conn:
            ...
            docs = [Document(page_content="这是一件：" + record["title"] + "。衣服属性为：" + record["description"] + record["marketing"] + datetime.now().strftime('%Y-%m-%d-%H%M%S'), 
                    ...
                    with cols[1]:
                        with st.popover("📝修改信息"):
                            with st.form(f"edit_product_{row.iloc[0]}_form"):
                                ...
                                product_marketing = st.text_area(label="种草文案", key=f"product_marketing_{row.iloc[0]}", value=row.iloc[10])
...
if __name__ == '__main__':
    if "ask_product_history" in st.session_state.keys():
        del st.session_state["ask_product_history"]
    if "ask_product_talker" in st.session_state.keys():
        del st.session_state["ask_product_talker"]
    if "ask_product_media" in st.session_state.keys():
    ...
    if st.session_state.rolename == "卖家":
        with st.expander("录入商品"):
            with st.form("add_product_info", clear_on_submit=False):
                ...
                cols = st.columns(3)
                with cols[0]:
                    marketing = st.form_submit_button("生成种草文案", use_container_width=True)
                    if marketing:
                        localdir = f"users/{st.session_state.username}/products/market"
                        os.makedirs(localdir, exist_ok=True)
                        filename = f"{datetime.now().strftime('%Y-%m-%d-%H%M%S')}-{product_image.name}"
                        Image.open(product_image).save(f"{localdir}/{filename}")
                        st.session_state.input_product_marketing = image_chat_answer(localdir, filename)
                product_marketing = st.text_area("种草文案", key="input_product_marketing")
